  ...
  async endSeasonAndCreateNew(seasonId: string): Promise<{
    ...
    try {
      ...
      const newSeasonResult = await client.query(
        `INSERT INTO seasons (year, season_code, name, display_name, status, current_phase, start_date, end_date)
         VALUES ($1, $2, $3, $4, 'active', 'spring', NOW(), NOW() + INTERVAL '1 year')
         RETURNING id`,
        [newYear, newSeasonCode, `${newSeasonCode}赛季`, `${newYear}赛季`]
      );
      ...
      // 7. 创建新赛季的春季赛（一个competition包含所有队伍）
      const springCompetitionResult = await client.query(
        `INSERT INTO competitions (season_id, name, type, status, format, scoring_rules, max_teams, start_date, end_date)
         VALUES ($1, $2, 'spring', 'planning', $3, $4, 40, NOW(), NOW() + INTERVAL '3 months')
         RETURNING id`,
        [
          newSeasonId,
          `${newSeasonCode} 春季赛`,
      ...
      const springCompetitionId = springCompetitionResult.rows[0].id;
